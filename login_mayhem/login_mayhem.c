...
password_node *in_hash_table(password_node *hash_table[], const char *find) {
    unsigned password_code = oaat(find, strlen(find), NUM_BITS);
    password_node *password_ptr = hash_table[password_code];
    while (password_ptr) {
...
int already_added(char all_substrings[][MAX_PASSWORD + 1], int total_substrings, const char *find) {
    for (int i = 0; i < total_substrings; i++) {
        if (strcmp(all_substrings[i], find) == 0) return 1;
    }
    return 0;
}
...
int main(void) {
    ...
    for (int op = 0; op < num_ops; op++) {
        ...
        if (op_type == 1) {
            ...
            password_node *password_ptr = in_hash_table(hash_table, password);
            if (!password_ptr) printf("0\n");
            else printf("%d\n", password_ptr->total);
        }
    }
    return 0;
